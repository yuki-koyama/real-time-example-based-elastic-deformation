...
public:
    ...
    //explicit Matrix(const Matrix<OtherScalar, NRow, NCol>& src) { operator=(src); }        // this constructor may lead to bugs...
    ...
    template <int N>
    explicit Matrix(const Vector<Scalar, N>& diagonal) { setDiagonal(diagonal); }
    explicit Matrix(const Vector<Scalar, NCol>& vecRow, const Vector<Scalar, NRow>& vecCol) { setTensorProd(vecRow, vecCol); }
    ~Matrix() {}
    ...
    template <int N>
    void setDiagonal(const Vector<Scalar, N>& diagonal) {
        assert((N == NRow && NRow <= NCol) || (N == NCol && NCol <= NRow));
        fill(0);
        for (size_t i = 0; i < N; ++i) (*this)(i, i) = diagonal[i];
    }
    void setTensorProd(const Vector<Scalar, NCol>& vecRow, const Vector<Scalar, NRow>& vecCol) {
        for (size_t i = 0; i < NRow; ++i) for (size_t j = 0; j < NCol; ++j) values_[NCol * i + j] = vecRow[j] * vecCol[i];
    }
    ...
    void setCol(size_t colIndex, const Vector<Scalar, NRow>& colValue) {
        for (size_t row = 0; row < NRow; ++row) (*this)(row, colIndex) = colValue[row];
    }
    ...
    Vector<Scalar, NRow> getCol(size_t colIndex) const {
        ...
        for (size_t row = 0; row < NRow; ++row) result[row] = (*this)(row, colIndex);
        return result;
    }
    ...
    // type conversion ==========================
    template<typename OtherScalar>
    Matrix<OtherScalar, NRow, NCol> convert() const {
        Matrix<OtherScalar, NRow, NCol> result;
        for (size_t i = 0; i < NRow * NCol; ++i) result[i] = static_cast<OtherScalar>(values_[i]);
        return result;
    }
    ...
    const value_type& operator()(size_t rowIndex, size_t colIndex) const { return values_[NCol * rowIndex + colIndex]; }
    
    // comparison ======================
    bool operator==(const matrix_type& rhs) const {
        for (size_t i = 0; i < NRow * NCol; ++i) if (values_[i] != rhs.values_[i]) return false;
        return true;
    }
    ...
    // arithmetic (member) =====================
    // with scalar
    matrix_type& operator+=(const value_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] += rhs; return *this; }
    matrix_type& operator-=(const value_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] -= rhs; return *this; }
    matrix_type& operator*=(const value_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] *= rhs; return *this; }
    matrix_type& operator/=(const value_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] /= rhs; return *this; }
    ...
    matrix_type& operator+=(const matrix_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] += rhs.values_[i]; return *this; }
    matrix_type& operator-=(const matrix_type& rhs) { for (size_t i = 0; i < NRow * NCol; ++i) values_[i] -= rhs.values_[i]; return *this; }
    ...
    Vector<Scalar, NRow> operator*(const Vector<Scalar, NCol>& rhs) const {
        Vector<Scalar, NRow> result;
        for (size_t row = 0; row < NRow; ++row) for (size_t col = 0; col < NCol; ++col) result[row] += (*this)(row, col) * rhs[col];
        return result;
    }
    // mult with matrix
    template<int NColNew>
    Matrix<Scalar, NRow, NColNew> operator*(const Matrix<Scalar, NCol, NColNew>& rhs) { 
        Matrix<Scalar, NRow, NColNew> result;
        for (size_t row = 0; row < NRow; ++row) for (size_t col = 0; col < NColNew; ++col) {
            Scalar sum = 0;
            for (size_t k = 0; k < NCol; ++k) sum += (*this)(row, k) * rhs(k, col);
            result(row, col) = sum;
        }
        return result;
    }
    ...
    matrix_type operator-() const {
        matrix_type result;
        for (size_t i = 0; i < NRow * NCol; ++i) result.values_[i] = -values_[i];
        return result;
    }
    ...
    Matrix<Scalar, NCol, NRow> transpose() const {
        ...
        for (size_t row = 0; row < NRow; ++row) for (size_t col = 0; col < NCol; ++col) result(col, row) = operator()(row, col);
        return result;
    }
...
// arithmetic (global) ====================
template <typename Scalar, int NRow, int NCol> Matrix<Scalar, NRow, NCol> operator+(const Scalar& lhs, const Matrix<Scalar, NRow, NCol>& rhs) { return rhs + lhs; }
template <typename Scalar, int NRow, int NCol> Matrix<Scalar, NRow, NCol> operator-(const Scalar& lhs, const Matrix<Scalar, NRow, NCol>& rhs) { return rhs - lhs; }
template <typename Scalar, int NRow, int NCol> Matrix<Scalar, NRow, NCol> operator*(const Scalar& lhs, const Matrix<Scalar, NRow, NCol>& rhs) { return rhs * lhs; }
template <typename Scalar, int NRow, int NCol> Matrix<Scalar, NRow, NCol> operator/(const Scalar& lhs, const Matrix<Scalar, NRow, NCol>& rhs) { return rhs / lhs; }
...
Vector<Scalar, NCol> operator*(const Vector<Scalar, NRow>& lhs, const Matrix<Scalar, NRow, NCol>& rhs) {
    Vector<Scalar, NCol> result;
    for (size_t col = 0; col < NCol; ++col) for (size_t row = 0; row < NRow; ++row) result[col] += lhs[row] * rhs(row, col);
    return result;
}
...
Matrix<Scalar, NRow, NCol> mult_col_row(const Vector<Scalar, NCol>& col, const Vector<Scalar, NRow>& row) {
    ...
    for (size_t i = 0; i < NRow; ++i) for (size_t j = 0; j < NCol; ++j) result(i, j) = row[i] * col[j];
    return result;
}
...
std::ostream& operator<<(std::ostream& os, const Matrix<Scalar, NRow, NCol>& rhs) {
    for (size_t i = 0; i < NRow; ++i) {
        for (size_t j = 0; j < NCol - 1; ++j) os << rhs(i, j) << '\t';
        os << rhs(i, NCol - 1) << '\n';
    }
    return os;
}

#include "private/Matrix1x1.h"
#include "private/Matrix2x2.h"
#include "private/Matrix3x3.h"
#include "private/Matrix4x4.h"
