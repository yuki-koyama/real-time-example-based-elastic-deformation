...
struct Gui {
    ...
    struct General {
        ...
        General()
            : m_bar(0)
            , m_demoMode(DEMOMODE_AUTHOR)
            , m_renderMode(RENDERMODE_TRIMESH)
    ...
    struct Author {
        ...
        Author()
            : m_bar(0)
            , m_manipulateMode(MANIPULATEMODE_TRANSLATE)
        {}
    ...
    struct Effect {
        ...
        void init();
        Effect() : m_sm_contrast(1.2f),
        m_sm_lightness(0.40f),
        m_fixed_contrast(1.2f),
        m_fixed_lightness(0.55f)
        {}
    } m_effect;
