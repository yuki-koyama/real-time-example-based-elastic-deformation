...
public:
    ...
    // type conversion ==========================
    template<typename OtherScalar>
    Vector<OtherScalar, 4> convert() const {
    ...
    bool equals(const vector_type& rhs, Scalar epsilon = std::numeric_limits<Scalar>::epsilon()) const {
        ...
        if (d.x_ <= -epsilon || epsilon <= d.x_ ||
            d.y_ <= -epsilon || epsilon <= d.y_ ||
            d.z_ <= -epsilon || epsilon <= d.z_ ||
            d.w_ <= -epsilon || epsilon <= d.w_)
            return false;
        return true;
    }
    ...
    bool operator<(const vector_type& rhs) const {
        ...
        if (z_ < rhs.z_) return true; else if (rhs.z_ < z_) return false;
    ...
    // arithmetic (member) =====================
    // with scalar
    vector_type& operator+=(value_type rhs) { x_ += rhs; y_ += rhs; z_ += rhs; w_ += rhs; return *this; }
    vector_type& operator-=(value_type rhs) { x_ -= rhs; y_ -= rhs; z_ -= rhs; w_ -= rhs; return *this; }
    vector_type& operator*=(value_type rhs) { x_ *= rhs; y_ *= rhs; z_ *= rhs; w_ *= rhs; return *this; }
    vector_type& operator/=(value_type rhs) { x_ /= rhs; y_ /= rhs; z_ /= rhs; w_ /= rhs; return *this; }
    ...
    vector_type operator/(value_type rhs) const { return vector_type(*this) /= rhs; }
    // with vector
    vector_type& operator+=(const vector_type& rhs) { x_ += rhs.x_; y_ += rhs.y_; z_ += rhs.z_; w_ += rhs.w_; return *this; }
    vector_type& operator-=(const vector_type& rhs) { x_ -= rhs.x_; y_ -= rhs.y_; z_ -= rhs.z_; w_ -= rhs.w_; return *this; }
    vector_type& operator*=(const vector_type& rhs) { x_ *= rhs.x_; y_ *= rhs.y_; z_ *= rhs.z_; w_ *= rhs.w_; return *this; }
    vector_type& operator/=(const vector_type& rhs) { x_ /= rhs.x_; y_ /= rhs.y_; z_ /= rhs.z_; w_ /= rhs.w_; return *this; }
    ...
    vector_type operator-() const {
        return vector_type(-x_, -y_, -z_, -w_);
    }
    ...
    // append ====================
    template <int M>
    Vector<Scalar, 4 + M> append(const Vector<Scalar, M>& tail) const {
