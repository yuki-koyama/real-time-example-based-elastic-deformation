...
bool TetMesh::load(const string& filename_base) {
    tetgenio in;
    if (!in.load_tetmesh(const_cast<char*>(filename_base.c_str())))
        return false;
    init(in);
    return true;
}
...
void TetMesh::remove_unused_vertices() {
    ...
    for (vector<Vertex>::iterator v = vertices_.begin(); v != vertices_.end(); ++v)
        for (list<int>::iterator neighbor_vid = v->neighbor_vid_.begin(); neighbor_vid != v->neighbor_vid_.end(); ++neighbor_vid)
