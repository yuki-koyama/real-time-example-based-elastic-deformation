...
public:
    ...
    // type conversion ==========================
    template<typename OtherScalar>
    Vector<OtherScalar, 3> convert() const {
    ...
    // arithmetic (member) =====================
    // with scalar
    vector_type& operator+=(value_type rhs) { x_ += rhs; y_ += rhs; z_ += rhs; return *this; }
    vector_type& operator-=(value_type rhs) { x_ -= rhs; y_ -= rhs; z_ -= rhs; return *this; }
    vector_type& operator*=(value_type rhs) { x_ *= rhs; y_ *= rhs; z_ *= rhs; return *this; }
    vector_type& operator/=(value_type rhs) { x_ /= rhs; y_ /= rhs; z_ /= rhs; return *this; }
    ...
    vector_type operator/(value_type rhs) const { return vector_type(*this) /= rhs; }
    // with vector
    vector_type& operator+=(const vector_type& rhs) { x_ += rhs.x_; y_ += rhs.y_; z_ += rhs.z_; return *this; }
    vector_type& operator-=(const vector_type& rhs) { x_ -= rhs.x_; y_ -= rhs.y_; z_ -= rhs.z_; return *this; }
    vector_type& operator*=(const vector_type& rhs) { x_ *= rhs.x_; y_ *= rhs.y_; z_ *= rhs.z_; return *this; }
    vector_type& operator/=(const vector_type& rhs) { x_ /= rhs.x_; y_ /= rhs.y_; z_ /= rhs.z_; return *this; }
    ...
    friend value_type dot(const vector_type& rhs1, const vector_type& rhs2) { return (rhs1) | (rhs2); } // for OpenMesh compatibility
...
//    void normalize()  { (*this) /= norm(); }                  // for OpenMesh compatibility!
    ...
    // append ====================
    template <int M>
    Vector<Scalar, 3 + M> append(const Vector<Scalar, M>& tail) const {
