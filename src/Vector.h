...
namespace Y
{
    ...
    template <typename Scalar, int N> class Vector
    {
    public:
        ...
        Vector(value_type s0, value_type s1, value_type s2, value_type s3, value_type s4, value_type s5)
        {
            set(s0, s1, s2, s3, s4, s5);
        }
        Vector(value_type s0, value_type s1, value_type s2, value_type s3, value_type s4, value_type s5, value_type s6)
        {
            set(s0, s1, s2, s3, s4, s5, s6);
        }
        Vector(value_type s0,
               value_type s1,
               value_type s2,
               value_type s3,
               value_type s4,
        ...
        template <int M> Vector(const Vector<Scalar, M>& v_head, const Vector<Scalar, N - M>& v_tail)
        {
            set<M>(v_head, v_tail);
        }
        ...
        void
        set(value_type s0, value_type s1, value_type s2, value_type s3, value_type s4, value_type s5, value_type s6)
        {
            assert(N == 7);
            values_[0] = s0;
            values_[1] = s1;
        ...
        //    values_[N - 1] = s_tail;
        //}
        template <int M> void set(const Vector<Scalar, M>& v_head, const Vector<Scalar, N - M>& v_tail)
        {
            assert(M < N);
        ...
        // type conversion ==========================
        template <typename OtherScalar> Vector<OtherScalar, N> convert() const
        {
        ...
        value_type elemMin() const
        {
            ...
            for (size_t i = 0; i < N; ++i)
                result = values_[i] < result ? values_[i] : result;
            return result;
        }
        value_type elemMax() const
        {
            ...
            for (size_t i = 0; i < N; ++i)
                result = result < values_[i] ? values_[i] : result;
            return result;
        }
        ...
        vector_type pairwiseMax(const vector_type& rhs) const
        {
            vector_type result;
            for (size_t i = 0; i < N; ++i)
                result.values_[i] = values_[i] < rhs.values_[i] ? rhs.values_[i] : values_[i];
            return result;
        }
        ...
        // append ====================
        template <int M> Vector<Scalar, N + M> append(const Vector<Scalar, M>& tail) const
        {
    ...
    // arithmetic (global) ====================
    template <typename Scalar, int N> Vector<Scalar, N> operator+(const Scalar& lhs, const Vector<Scalar, N>& rhs)
    {
        return rhs + lhs;
    }
    template <typename Scalar, int N> Vector<Scalar, N> operator-(const Scalar& lhs, const Vector<Scalar, N>& rhs)
    ...
    // cross product==========================
    template <typename Scalar> Vector<Scalar, 3> operator%(const Vector<Scalar, 3>& lhs, const Vector<Scalar, 3>& rhs)
    {
        return Vector<Scalar, 3>(
            lhs[1] * rhs[2] - lhs[2] * rhs[1], lhs[2] * rhs[0] - lhs[0] * rhs[2], lhs[0] * rhs[1] - lhs[1] * rhs[0]);
    }
    template <typename Scalar>
    Vector<Scalar, 3> cross_product(const Vector<Scalar, 3>& lhs, const Vector<Scalar, 3>& rhs)
    {
        return lhs % rhs;
    }

    // norm
    template <typename Scalar> Scalar norm(const Vector<Scalar, 3>& src) { return src.norm(); }

    template <typename Scalar> Vector<Scalar, 3> normalize(const Vector<Scalar, 3>& src) { return src.normalized(); }

    template <typename Scalar> void vectorize(Vector<Scalar, 3>& src, Scalar val)
    {
        src[0] = val;
        src[1] = val;
        src[2] = val;
    ...
    template <typename Scalar> Vector<Scalar, 2> rotate90(const Vector<Scalar, 2>& src)
    {
        return Vector<Scalar, 2>(-src[1], src[0]);
    }

    // abs =================================================
    template <typename Scalar, int N> Vector<Scalar, N> abs(const Vector<Scalar, N>& arg)
    {
    ...
    // stream I/O ===========================
    template <typename Scalar, int N> std::ostream& operator<<(std::ostream& os, const Vector<Scalar, N>& rhs)
    {
        for (size_t i = 0; i < N - 1; ++i)
            os << rhs[i] << '\t';
        os << rhs[N - 1];
    ...
    template <typename Scalar, int N> std::istream& operator>>(std::istream& is, Vector<Scalar, N>& rhs)
    {
        for (size_t i = 0; i < N; ++i)
            is >> rhs[i];
        return is;
    ...
    template <typename Scalar> Vector<Scalar, 3> cross(const Vector<Scalar, 3>& lhs, const Vector<Scalar, 3>& rhs)
    {
        return lhs % rhs;
    }
    template <typename Scalar, int N> Vector<Scalar, N> vectorized(const Scalar& arg) { return Vector<Scalar, N>(arg); }
