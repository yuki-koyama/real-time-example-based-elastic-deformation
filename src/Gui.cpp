...
{
    if (!core.m_scene.m_isPause) return;
...
{
    if (core.m_scene.m_walls.empty()) return;
...
void TW_CALL Gui::General::CBSet_wall_load(const void* value, void* clientData)
{
    ...
    if (gui.m_general.m_wall_fname_load.empty()) return;
    Wall wall;
    if (!wall.loadXML(gui.m_general.m_wall_fname_load + ".xml")) return;
    core.m_scene.m_walls.push_back(wall);
...
void TW_CALL Gui::General::CBSet_wall_save(const void* value, void* clientData)
{
    ...
    if (gui.m_general.m_wall_fname_save.empty()) return;
    Wall& wall = core.m_scene.m_walls.back();
    if (!wall.saveXML(gui.m_general.m_wall_fname_save + ".xml")) return;
    gui.m_general.m_wall_fname_save.clear();
}
void TW_CALL Gui::General::CB_wall_delete(void*)
{
    if (!core.m_scene.m_walls.empty()) core.m_scene.m_walls.pop_back();
}
// authoring mode
void TW_CALL Gui::Author::CBSet_xml_load(const void* value, void* clientData)
{
    ...
    if (gui.m_author.m_xml_load_fname.empty()) return;
    if (!smObject_author.loadXML(gui.m_author.m_xml_load_fname + ".xml")) return;
...
void TW_CALL Gui::Author::CBSet_xml_save(const void* value, void* clientData)
{
    ...
    if (gui.m_author.m_xml_save_fname.empty()) return;
...
void TW_CALL Gui::Author::CBSet_tetmesh(const void* value, void* clientData)
{
    ...
    if (!smObject_author.tetmesh.load(gui.m_author.m_tetmesh_fname)) return;
...
void TW_CALL Gui::Author::CB_constraint_delete(void*)
{
    ...
    if (currentConstraintID == -1) return;
    ...
    for (int i = 0; i < smObject_author.particles.size(); ++i)
    {
        ...
        if (currentConstraintID < p.constraintID) --p.constraintID;
    }
...
{
    if (smObject_author.particles.front().x_example.size() == 1) return;
    for (int i = 0; i < smObject_author.particles.size(); ++i)
...
{
    if (smObject_author.tetmesh.empty()) return;
    gui.m_author.m_example_tetnode_load_fname = *reinterpret_cast<const string*>(value);
    if (gui.m_author.m_example_tetnode_load_fname.empty()) return;
    if (!smObject_author.loadNode(gui.m_author.m_example_tetnode_load_fname)) return;
...
void TW_CALL Gui::Author::CBSet_example_tetnode_save(const void* value, void* clientData)
{
    ...
    if (gui.m_author.m_example_tetnode_save_fname.empty()) return;
    if (!smObject_author.saveNode(gui.m_author.m_example_tetnode_save_fname)) return;
...
void TW_CALL Gui::Play::CBSet_scene_load(const void* value, void* clientData)
{
    ...
    if (gui.m_general.m_demoMode != Gui::General::DEMOMODE_PLAY || gui.m_play.m_scene_load_fname.empty()) return;
    if (!core.m_scene.loadXML(gui.m_play.m_scene_load_fname + ".xml")) return;
...
void TW_CALL Gui::Play::CBSet_scene_save(const void* value, void* clientData)
{
    ...
    if (gui.m_general.m_demoMode != Gui::General::DEMOMODE_PLAY || gui.m_play.m_scene_save_fname.empty()) return;
    if (!core.m_scene.saveXML(gui.m_play.m_scene_save_fname + ".xml")) return;
...
void TW_CALL Gui::Play::CBSet_object_load(const void* value, void* clientData)
{
    ...
    if (gui.m_general.m_demoMode != Gui::General::DEMOMODE_PLAY || gui.m_play.m_object_fname.empty()) return;
    ShapeMatchingObject object;
    if (!object.loadXML(gui.m_play.m_object_fname + ".xml")) return;
...
void TW_CALL Gui::Param::CBSet_load(const void* value, void* clientData)
{
    ...
    if (gui.m_param.m_load_fname.empty()) return;
    if (!core.m_physParam.loadXML(gui.m_param.m_load_fname + ".xml")) return;
...
void TW_CALL Gui::Param::CBSet_save(const void* value, void* clientData)
{
    ...
    if (gui.m_param.m_save_fname.empty()) return;
    if (!core.m_physParam.saveXML(gui.m_param.m_save_fname + ".xml")) return;
...
void TW_CALL Gui::Param::CBSet_mass(const void* value, void* clientData)
{
    ...
    else if (core.m_gui.m_general.m_demoMode == Gui::General::DEMOMODE_PLAY)
    {
        for (list<ShapeMatchingObject>::iterator smObject = core.m_scene.m_smObjects_play.begin();
             smObject != core.m_scene.m_smObjects_play.end();)
        {
...
void Gui::General::init()
{
    ...
    TwAddVarRW(m_bar,
               "demomode",
               TwDefineEnum("DemoModeType", 0, 0),
               &m_demoMode,
               "label='demo mode' enum='0 {authoring mode}, 1 {playing mode}'");
    TwAddVarRW(m_bar,
               "render_mode",
               TwDefineEnum("RenderModeType", 0, 0),
               &m_renderMode,
               "group='render' label='mode' enum='0 {both}, 1 {particle}, 2 {trimesh}' key=v");
#if !SIMPLE_MODE
    ...
    TwAddVarCB(m_bar,
               "render_lightPos",
               TW_TYPE_DIR3F,
               CBSet_render_lightPos,
               CBGet_render_lightPos,
               0,
               "group='render' label='light pos'");
    TwAddVarRW(m_bar,
               "simulation_pause",
               TW_TYPE_BOOLCPP,
               &core.m_scene.m_isPause,
               "group='simulation' label='run/pause' true='paused' false='running' key=p");
    ...
    TwAddVarRW(m_bar,
               "simulation_useGravity",
               TW_TYPE_BOOLCPP,
               &core.m_scene.m_useGravity,
               "group='simulation' label='gravity' key=g");
#if !SIMPLE_MODE
    ...
    TwAddVarCB(
        m_bar, "wall_normal", TW_TYPE_DIR3D, CBSet_wall_normal, CBGet_wall_normal, 0, "group='wall' label='normal'");
    TwAddVarCB(m_bar,
               "wall_distance",
               TW_TYPE_DOUBLE,
               CBSet_wall_distance,
               CBGet_wall_distance,
               0,
               "group='wall' label='distance' step=0.01");
    TwAddVarCB(
        m_bar, "wall_color", TW_TYPE_COLOR3F, CBSet_wall_color, CBGet_wall_color, 0, "group='wall' label='color'");
    TwAddVarCB(m_bar,
               "wall_reflection",
               TW_TYPE_FLOAT,
               CBSet_wall_reflection,
               CBGet_wall_reflection,
               0,
               "group='wall' label='reflection' min=0.0 max=1.0 step=0.01");
    TwAddVarCB(m_bar,
               "wall_friction",
               TW_TYPE_FLOAT,
               CBSet_wall_friction,
               CBGet_wall_friction,
               0,
               "group='wall' label='friction' min=0.0 max=2.0 step=0.01");
    TwAddVarCB(m_bar,
               "wall_repulsion",
               TW_TYPE_FLOAT,
               CBSet_wall_repulsion,
               CBGet_wall_repulsion,
               0,
               "group='wall' label='repulsion' min=0.0 max=5.0 step=0.01");
    TwAddVarCB(m_bar,
               "wall_load",
               TW_TYPE_STDSTRING,
               CBSet_wall_load,
...
void Gui::Author::init()
{
    ...
    TwAddVarCB(
        m_bar, "xml_load", TW_TYPE_STDSTRING, CBSet_xml_load, CBGet_stdstring, &m_xml_load_fname, "label='loadXML'");
    TwAddVarCB(
        m_bar, "xml_save", TW_TYPE_STDSTRING, CBSet_xml_save, CBGet_stdstring, &m_xml_save_fname, "label='saveXML'");
    TwAddVarCB(
        m_bar, "tetmesh", TW_TYPE_STDSTRING, CBSet_tetmesh, CBGet_stdstring, &m_tetmesh_fname, "label='tetmesh'");
    TwAddVarCB(
        m_bar, "trimesh", TW_TYPE_STDSTRING, CBSet_trimesh, CBGet_stdstring, &m_trimesh_fname, "label='trimesh'");
    // constraint
    TwAddVarRW(m_bar,
               "constraint_mode",
               TwDefineEnum("ManipulateModeType", 0, 0),
               &m_manipulateMode,
               "group='constraint' label='manip mode' enum='0 {translate}, 1 {rotate}, 2 {scale}' key=m");
    ...
    TwAddVarCB(m_bar,
               "example_tetnode_load",
               TW_TYPE_STDSTRING,
               CBSet_example_tetnode_load,
               CBGet_stdstring,
               &m_example_tetnode_load_fname,
               "group='example' label='load pose'");
    TwAddVarCB(m_bar,
               "example_tetnode_save",
               TW_TYPE_STDSTRING,
               CBSet_example_tetnode_save,
               CBGet_stdstring,
               &m_example_tetnode_save_fname,
               "group='example' label='save pose'");
    // config
    TwAddVarRW(m_bar,
               "config_damping",
               TW_TYPE_FLOAT,
               &smObject_author.m_config.m_damping,
               "group='config' label='damping' min=0 max=1 step=0.01");
    TwAddVarRW(m_bar,
               "config_stiff_alpha",
               TW_TYPE_FLOAT,
               &smObject_author.m_config.m_stiffness_alpha,
               "group='config' label='stiff_alpha' min=0.00 max=2.00 step=0.01");
    TwAddVarRW(m_bar,
               "config_stiff_iter",
               TW_TYPE_INT32,
               &smObject_author.m_config.m_stiffness_iter,
               "group='config' label='stiff_iter' min=1 max=50");
    TwAddVarRW(m_bar,
               "config_useLocal",
               TW_TYPE_BOOLCPP,
               &smObject_author.m_config.m_useLocalExample,
               "group='config' label='local example'");
    TwAddVarRW(m_bar,
               "config_beta",
               TW_TYPE_FLOAT,
               &smObject_author.m_config.m_beta,
               "group='config' label='beta' min=0 max=2.0 step=0.005");
    // initial
    TwAddVarRW(m_bar,
               "initial_offXmin",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_offset.x_,
               "group='initial' label='offset x' step=0.1");
    TwAddVarRW(m_bar,
               "initial_offYmin",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_offset.y_,
               "group='initial' label='offset y' step=0.1");
    TwAddVarRW(m_bar,
               "initial_offZmin",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_offset.z_,
               "group='initial' label='offset z' step=0.1");
    //    TwAddVarRW(m_bar, "initial_offXmin", TW_TYPE_FLOAT, &smObject_author.m_initial.m_offset_min.x_,
    //    "group='initial' label='offset x min' step=0.1"); TwAddVarRW(m_bar, "initial_offYmin", TW_TYPE_FLOAT,
    //    &smObject_author.m_initial.m_offset_min.y_, "group='initial' label='offset y min' step=0.1");
    //    TwAddVarRW(m_bar, "initial_offZmin", TW_TYPE_FLOAT, &smObject_author.m_initial.m_offset_min.z_,
    //    "group='initial' label='offset z min' step=0.1"); TwAddVarRW(m_bar, "initial_offXmax", TW_TYPE_FLOAT,
    //    &smObject_author.m_initial.m_offset_max.x_, "group='initial' label='offset x max' step=0.1");
    //    TwAddVarRW(m_bar, "initial_offYmax", TW_TYPE_FLOAT, &smObject_author.m_initial.m_offset_max.y_,
    //    "group='initial' label='offset y max' step=0.1"); TwAddVarRW(m_bar, "initial_offZmax", TW_TYPE_FLOAT,
    //    &smObject_author.m_initial.m_offset_max.z_, "group='initial' label='offset z max' step=0.1");
    TwAddVarRW(m_bar,
               "initial_velX",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_velocity.x_,
               "group='initial' label='velocity x' step=0.1");
    TwAddVarRW(m_bar,
               "initial_velY",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_velocity.y_,
               "group='initial' label='velocity y' step=0.1");
    TwAddVarRW(m_bar,
               "initial_velZ",
               TW_TYPE_FLOAT,
               &smObject_author.m_initial.m_velocity.z_,
               "group='initial' label='velocity z' step=0.1");
    // color
    ...
    TwAddVarRO(m_bar,
               "performance_timeMin",
               TW_TYPE_DOUBLE,
               &smObject_author.m_performance.m_timeMin,
               "group='performance' label='min'");
    TwAddVarRO(m_bar,
               "performance_timeMax",
               TW_TYPE_DOUBLE,
               &smObject_author.m_performance.m_timeMax,
               "group='performance' label='max'");
    TwAddVarRO(m_bar,
               "performance_timeAvg",
               TW_TYPE_DOUBLE,
               &smObject_author.m_performance.m_timeAvg,
               "group='performance' label='avg'");
#endif
}
void Gui::Play::init()
{
...
void Gui::Param::init()
{
    ...
    // TwAddVarRW(m_bar, "frameFrequency", TW_TYPE_INT32, &core.m_physParam.m_frameFrequency, "label='frameFrequency'
    // min=1, max=1000");
    TwAddVarRW(m_bar, "dt", TW_TYPE_FLOAT, &core.m_physParam.m_dt, "label='dt' step=0.0001");
    // TwAddVarRW(m_bar, "stepPerFrame", TW_TYPE_INT32, &core.m_physParam.m_stepPerFrame, "label='stepPerFrame' min=1
    // max=100");
    TwAddVarRW(m_bar,
               "airResistance",
               TW_TYPE_FLOAT,
               &core.m_physParam.m_airResistance,
               "label='airResistance' min=0.001 max=0.1 step=0.001");
    TwAddVarRW(m_bar, "gravity", TW_TYPE_FLOAT, &core.m_physParam.m_gravity, "label='gravity' step = 0.1");
    TwAddVarRW(m_bar,
               "springCoef",
               TW_TYPE_FLOAT,
               &core.m_physParam.m_springCoef,
               "label='springCoef' min=10 max=10000 step=10");
    ...
    //    TwAddVarCB(m_bar, "mass", TW_TYPE_FLOAT, CBSet_mass, CBGet_mass, 0, "label='mass' step=0.05 min=0.05
    //    max=1000.0");
#endif
}
void Gui::Tmp::init()
...
void Gui::Effect::init()
{
    ...
    TwAddVarRW(
        m_bar, "fixed_contrast", TW_TYPE_FLOAT, &m_fixed_contrast, "label='fixed_contrast' step=0.05 min=0.0 max=5.0");
    TwAddVarRW(m_bar,
               "fixed_lightness",
               TW_TYPE_FLOAT,
               &m_fixed_lightness,
               "label='fixed_lightness' step=0.05 min=0.0 max=1.0");
#endif
}
void Gui::Camera::init()
{
    ...
    TwAddVarRW(m_bar,
               "camera_x",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.eyePoint.x_,
               "group='camera' label='x' step=0.05");
    TwAddVarRW(m_bar,
               "camera_y",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.eyePoint.y_,
               "group='camera' label='y' step=0.05");
    TwAddVarRW(m_bar,
               "camera_z",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.eyePoint.z_,
               "group='camera' label='z' step=0.05");
    TwAddVarRW(m_bar,
               "focus_x",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.focusPoint.x_,
               "group='focus' label='x' step=0.05");
    TwAddVarRW(m_bar,
               "focus_y",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.focusPoint.y_,
               "group='focus' label='y' step=0.05");
    TwAddVarRW(m_bar,
               "focus_z",
               TW_TYPE_DOUBLE,
               &core.m_drawer.m_trackball.focusPoint.z_,
               "group='focus' label='z' step=0.05");
}
