...
template <typename Scalar> class Vector<Scalar, 1>
{
public:
    ...
    // type conversion ==========================
    template <typename OtherScalar> Vector<OtherScalar, 1> convert() const
    {
        return Vector<OtherScalar, 1>(static_cast<OtherScalar>(value_));
    }
    // comparison ===============================
    ...
    // arithmetic (member) =====================
    // with scalar
    vector_type& operator+=(const value_type& rhs)
    {
        value_ += rhs;
        return *this;
    }
    ...
    {
        return vector_type(value_ < rhs.value_ ? rhs.value_ : value_);
    }
    ...
    // append ====================
    template <int M> Vector<Scalar, 1 + M> append(const Vector<Scalar, M>& tail) const
    {
