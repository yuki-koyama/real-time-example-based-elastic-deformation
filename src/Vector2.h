...
public:
    ...
    explicit Vector(const value_type v[2])
    {
        x_ = v[0];
        y_ = v[1];
    }
    ...
    // type conversion ==========================
    template <typename OtherScalar> Vector<OtherScalar, 2> convert() const
    {
    ...
    vector_type operator/(const vector_type& rhs) const { return vector_type(*this) /= rhs; }
    // negate
    vector_type operator-() const { return vector_type(-x_, -y_); }

    // vector operation =========================
    // dot product
    value_type operator|(const vector_type& rhs) const { return x_ * rhs.x_ + y_ * rhs.y_; }
    ...
    value_type  elemSum() const { return x_ + y_; }
    ...
    // append ====================
    template <int M> Vector<Scalar, 2 + M> append(const Vector<Scalar, M>& tail) const
    {
