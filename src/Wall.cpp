...
Wall::Coef::Coef() : m_reflection(0.6f), m_friction(1.0f), m_repulsion(2.0f) {}
...
bool Wall::loadXML(const string& fname)
{
    ...
    if (!loadXML(txe_wall)) return false;

    return true;
}
bool Wall::loadXML(const TiXmlElement* txe_wall)
{
    if (!txe_wall) return false;
    ...
    if (m_normal.length() == 0) return false;
    m_normal.normalize();
...
bool Wall::saveXML(const string& fname) const
{
    ...
    if (!saveXML(txe_wall)) return false;
...
Wall::Wall(const Y::Vector3d& normal, double distance, const Y::Vector3d& color)
    : m_normal(normal), m_distance(0), m_color(color)
{
    initShaders();
    update();
}
