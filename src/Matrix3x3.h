...
template <typename Scalar> class Matrix<Scalar, 3, 3>
{
public:
    ...
    template <typename OtherScalar> explicit Matrix(const Matrix<OtherScalar, 3, 3>& src) { operator=(src); }
    ...
    {
        set(s00, s01, s02, s10, s11, s12, s20, s21, s22);
    }
    ...
    void setTensorProd(const Vector<Scalar, 3>& vecRow, const Vector<Scalar, 3>& vecCol)
    {
        for (size_t i = 0; i < 3; ++i)
            for (size_t j = 0; j < 3; ++j)
                values_[3 * i + j] = vecRow[j] * vecCol[i];
    }
    void setDiagonal(const Vector<Scalar, 3>& diagonal)
    {
        set(diagonal[0], 0, 0, 0, diagonal[1], 0, 0, 0, diagonal[2]);
    }
    ...
    void
    setRows(const Vector<Scalar, 3>& rowValue0, const Vector<Scalar, 3>& rowValue1, const Vector<Scalar, 3>& rowValue2)
    {
        set(rowValue0[0],
            rowValue0[1],
            rowValue0[2],
    ...
    void
    setCols(const Vector<Scalar, 3>& colValue0, const Vector<Scalar, 3>& colValue1, const Vector<Scalar, 3>& colValue2)
    {
        set(colValue0[0],
            colValue1[0],
            colValue2[0],
    ...
    template <typename OtherScalar> matrix_type& operator=(const Matrix<OtherScalar, 3, 3>& src)
    {
    ...
    // mult with matrix
    template <int NColNew> Matrix<Scalar, 3, NColNew> operator*(const Matrix<Scalar, 3, NColNew>& rhs)
    {
    ...
    bool invert()
    {
        ...
        s = 1 / s;
        matrix_type m(*this);
        this->set(m(1, 1) * m(2, 2) - m(1, 2) * m(2, 1),
    ...
    matrix_type inverse() const
    {
        ...
        m2.set(m(1, 1) * m(2, 2) - m(1, 2) * m(2, 1),
               m(0, 2) * m(2, 1) - m(0, 1) * m(2, 2),
               m(0, 1) * m(1, 2) - m(0, 2) * m(1, 1),
               m(1, 2) * m(2, 0) - m(1, 0) * m(2, 2),
               m(0, 0) * m(2, 2) - m(0, 2) * m(2, 0),
