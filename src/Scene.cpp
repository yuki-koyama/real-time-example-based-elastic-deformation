...
bool Scene::loadXML(const string& fname)
{
    ...
    { // objects
        ...
        for (int i = 0; i < n_objects; ++i, txe_object = txe_object->NextSiblingElement())
            if (!m_smObjects_play_template[i].loadXML(txe_object)) return false;
    }
    ...
    { // walls
        ...
        for (int i = 0; i < n_walls; ++i, txe_wall = txe_wall->NextSiblingElement())
            if (!m_walls[i].loadXML(txe_wall)) return false;
    }
...
bool Scene::saveXML(const string& fname) const
{
    ...
    { // objects
        ...
        for (int i = 0; i < n_objects; ++i)
        {
            ...
            if (!m_smObjects_play_template[i].saveXML(txe_object)) return false;
        }
    }
    { // walls
        ...
        for (int i = 0; i < n_walls; ++i)
        {
            ...
            if (!m_walls[i].saveXML(txe_wall)) return false;
        }
    }
...
void Scene::simulateOneStep(float dt)
{
    ...
    {
        for (list<ShapeMatchingObject>::iterator smObject = m_smObjects_play.begin();
             smObject != m_smObjects_play.end();)
        {
...
Scene::Scene() : m_isPause(false), m_useGravity(true), m_lightPos(-3.0, 4.0, 6.0)
{
