...
void EventHandler::glut_mouse(int button, int state, int x, int y, int display_scale)
{
            ...
            { // DEMOMODE_PLAY
                ...
                if (core.m_gui.m_general.m_demoMode == Gui::General::DEMOMODE_AUTHOR)
                {
                    ...
                    for (TriMesh::FIter f = trimesh.faces_begin(); f != trimesh.faces_end(); ++f)
                    {
                        ...
                        if (!Y::Util::calcIntersectionLineTriangle(
                                line_v0, line_v1, face_w[0], face_w[1], face_w[2], baryCoordV, baryCoordW))
                            continue;
                        if (baryCoordW[0] < 0 || baryCoordW[1] < 0 || baryCoordW[2] < 0 || 1 < baryCoordW[0] ||
                            1 < baryCoordW[1] || 1 < baryCoordW[2])
                            continue;
                        ...
                        if (dist < picked_dist)
                        {
                            ...
                            for (std::vector<int>::iterator it =
                                     smObject.clusters[picked_particle->main_cluster_id].particle_ids.begin();
                                 it != smObject.clusters[picked_particle->main_cluster_id].particle_ids.end();
                                 ++it)
                            {
        ...
        { // (state == GLUT_UP)
            if (m_dragMode == DRAGMODE_SELECT)
            {
                ...
                for (int i = 0; i < smObject.particles.size(); ++i)
                {
                    ...
                    Y::Vector2d x2 = Y::GLUtil::project(x3).xy();
                    if (radius < (x2 - m_select_pos[0]).length()) continue;
                    if (p.constraintID != -1) return; // groups of constrained particles should not overlap
                }
    ...
    else if (button == GLUT_RIGHT_BUTTON)
    {
        if (state == GLUT_DOWN)
        {
            core.m_drawer.m_trackball.mouseDown(x,
                                                y,
                                                flagShift ? Y::Trackball::DRAGMODE_ZOOM
                                                          : flagCtrl ? Y::Trackball::DRAGMODE_PAN
                                                                     : Y::Trackball::DRAGMODE_ROTATE);
        }
        else
        {
