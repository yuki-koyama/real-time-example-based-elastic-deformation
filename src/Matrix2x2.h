...
template <typename Scalar> class Matrix<Scalar, 2, 2>
{
public:
...
public:
    ...
    template <typename OtherScalar> explicit Matrix(const Matrix<OtherScalar, 2, 2>& src) { operator=(src); }
    ...
    Matrix(const value_type& s00, const value_type& s01, const value_type& s10, const value_type& s11)
    {
        set(s00, s01, s10, s11);
    }
    // dimension-wise setters =================
    void set(const value_type& s00, const value_type& s01, const value_type& s10, const value_type& s11)
    {
        assert(2 == 2 && 2 == 2);
        values_[0] = s00;
        values_[1] = s01;
    ...
    void setTensorProd(const Vector<Scalar, 2>& vecRow, const Vector<Scalar, 2>& vecCol)
    {
        for (size_t i = 0; i < 2; ++i)
            for (size_t j = 0; j < 2; ++j)
                values_[2 * i + j] = vecRow[j] * vecCol[i];
    }
    void setDiagonal(const Vector<Scalar, 2>& diagonal) { set(diagonal[0], 0, 0, diagonal[1]); }
    void setIdentity() { set(1, 0, 0, 1); }
    ...
    void setRows(const Vector<Scalar, 2>& rowValue0, const Vector<Scalar, 2>& rowValue1)
    {
        set(rowValue0[0], rowValue0[1], rowValue1[0], rowValue1[1]);
    }
    void setCols(const Vector<Scalar, 2>& colValue0, const Vector<Scalar, 2>& colValue1)
    {
        set(colValue0[0], colValue1[0], colValue0[1], colValue1[1]);
    }
    ...
    template <typename OtherScalar> matrix_type& operator=(const Matrix<OtherScalar, 2, 2>& src)
    {
    ...
    // mult with matrix
    template <int NColNew> Matrix<Scalar, 2, NColNew> operator*(const Matrix<Scalar, 2, NColNew>& rhs)
    {
    ...
    bool invert()
    {
        ...
        s = 1 / s;
        matrix_type m(*this);
        this->set(m(1, 1), -m(0, 1), -m(1, 0), m(0, 0));
        (*this) *= s;
        return true;
    }
