...
namespace Y
{
    ...
    // "with precision" means that they use gettimeofday instead of clock(), so they can measuer smaller time than tick
    // time.
    ...
    public:
        ClockDataWithPrecision(const std::string& clkName = "", bool showPercent = true)
            : clkName_(clkName), showPercent_(showPercent)
        {
        }
        ...
        void setShowPercent(bool showPercent) { showPercent_ = showPercent; }
        void print()
        {
            double total = 0.0;
            ...
            std::cout << "[";
            if (!clkName_.empty()) std::cout << clkName_ << " | ";
            for (size_t i = 0; i < ticks_.size(); ++i)
            {
                std::cout << names_[i] << ":";
                if (showPercent_)
    ...
    public:
        ClockStartWithPrecision(ClockDataWithPrecision* clkData,
                                int                     currentIndex = 0,
                                bool                    autoPrint    = false,
                                bool                    resetTick    = false)
            : clkData_(clkData), currentIndex_(currentIndex), autoPrint_(autoPrint), clk_(clock())
        {
            if (autoPrint_) std::cout << "clock started: " << clkData_->getClkName() << std::endl;
            if (resetTick) clkData_->resetTick();
        }
        ...
        void print()
        {
            ...
            for (size_t i = 0; i < ticks_.size(); ++i)
            {
                ...
                else
                {
                    ...
                    printf(" %.3f msec, ", t);
                    //                    std::cout << (1000 * ticks_[i] / static_cast<double>(CLOCKS_PER_SEC)) <<
                    //                    "msec, ";
                }
            }
    ...
    public:
        ClockSimple(const std::string& message = "") : message_(message), clk_(clock())
        {
        ...
        {
            std::cout << "done!"
                      << "(" << (1000 * (clock() - clk_) / CLOCKS_PER_SEC) << "msec)\n";
        }
    };
    ...
    public:
        ClockLoop() : tick_(0), tickMax_(0), printIntervalMilliSec_(0), clk_(0), clk_init_(0) {}
